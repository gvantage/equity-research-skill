...
**第三步：技术分析**（仅对通过基本面者抓日线计算，省请求）。基于新浪日线（默认 120 根）计算：

- **MACD**（12/26/9）：DIF/DEA/柱、最近 5 日**金叉/死叉**；
- **MACD 背离**：比较最近两个价格波谷/波峰与对应 DIF —— 价创新低而 MACD 抬高=**底背离(看多)**；价创新高而 MACD 走低=**顶背离(看空)**；
- **均线**：MA20、MA60，价是否站上 MA20、是否均线多头（MA20>MA60）；
- **RSI14**、**20 日动量**。

由上述信号得**技术分**（0.35 中性基线上下浮动，`[0,1]`）：底背离/金叉/红柱/站上MA20/均线多头/RSI 超卖加分；顶背离/死叉/RSI 超买减分。

**第四步：综合评分排序**。`综合评分 = (1 - tech_weight)×价值分 + tech_weight×技术分`，其中**价值分** = `0.6×低PE + 0.4×低PB` 的百分位打分，`tech_weight` 默认 `0.4`（`--tech-weight` 可调）。**降序取前 `--count`（默认 10）只**。

个股名称/PE/PB/市值一律**以实时行情为准**。报告顶部“选股标准与筛选”一节展示标准、权重与漏斗（候选池 → 通过基本面 → 带看多信号 → 入选）；组合速览表含**排名/技术信号/综合评分**列；个股详情含**技术面**（MACD/背离/均线/RSI/动量/技术评分）与 LLM 的**技术面解读**。技术指标也会随基本面一起喂给 LLM，使结论兼顾两者。

相关参数：`--tech-weight`（技术面权重，默认 0.4）、`--require-bullish`（仅保留带看多技术信号：底背离/金叉/站上MA20）、`--pe-max/--pb-max/--min-mktcap/--include-st`。

> 其它选股方式：`--select rotate` 按日期确定性轮换（`offset = (date.toordinal()*count) % N` 环形取 N 只）；`--tickers sh600519,sz000858` 手动指定标的（这两种方式仍会计算技术指标并纳入研究）。用 `--count N` 调整数量。
