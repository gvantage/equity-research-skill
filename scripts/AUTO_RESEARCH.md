...
**第一步：全市场两阶段漏斗（默认 `--universe-source market`）**。A 股有 5000+ 只，管线**覆盖全市场**而非固定股票池：
- 从新浪全市场行情（`hs_a`，全部A股）按**总市值降序**分页拉取，跌破 `--min-mktcap` 即停（省请求）；用批量可得的 **PE / PB / 市值 / 换手率 / 今日涨幅** 做基本面初筛（约通过 800 只），再按 `0.45×低估值 + 0.35×高换手 + 0.20×今日涨幅` 初筛打分，取前 `--shortlist`（默认 80）只**入围**；
- 之后并入当日同花顺强势股（热点）、再对入围股抓日线做**深度多因子打分**。
- 新浪全市场不可用时自动降级到本地股票池 `a_share_universe.json`（`--universe-source file` 可强制用本地池）；本地池同时用作**行业标签映射**。
